...
from .utils import normalise_undirected_edges_by_labels
...
def sort_edges(edges, labelings, is_directed):
    ...
    for i in range(labelings.shape[0]):
        edge_class, mono = get_edge_id1(
            edge_with_node_labels[:, i * 2 : i * 2 + 2], order, np.empty(len(edges), dtype=np.uint32)
        )
...
# @njit
def rewire_fast(
    edges, edge_class, is_mono_color, block, is_directed, seed=None, num_flip_attempts_in=1, parallel=False
):

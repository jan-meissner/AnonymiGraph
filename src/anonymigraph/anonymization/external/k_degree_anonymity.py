# Implementation by Rossi: https://github.com/blextar/graph-k-degree-anonymity/tree/master
...
def _assignment_cost_additions_only(degree_sequence):
    return np.sum(degree_sequence[0] - degree_sequence)


# median of a sorted array
def _median(degree_sequence):
...
def _assignment_cost_additions_deletions(degree_sequence):
    md = _median(degree_sequence)
    return np.sum(np.abs(md - degree_sequence))


# Precomputation of the anonymisation cost, as described in Section 4 of [1]
def _anonymisation_cost_precomputation(degree_sequence, k, with_deletions):
    ...
    for i in range(n - 1):
        for j in range(i + k - 1, np.min([i + 2 * k, n])):
            if with_deletions:
                if C[i, j - 1] == np.inf:
                    C[i, j] = _assignment_cost_additions_deletions(degree_sequence[i : j + 1])
                else:
                    C[i, j] = C[i, j - 1] + _median(degree_sequence[i : j + 1]) - degree_sequence[j]
            else:
                if C[i, j - 1] == np.inf:
                    C[i, j] = _assignment_cost_additions_only(degree_sequence[i : j + 1])
                else:
...
def _dp_degree_anonymiser(degree_sequence, k, with_deletions=False):
    C = _anonymisation_cost_precomputation(degree_sequence, k, with_deletions)
    n = np.size(degree_sequence)
    Da = np.full(n, np.inf)
    sequences = [None] * n
    cost, anonymised_sequence = _dp_degree_anonymiser_recursion(degree_sequence, k, C, n, Da, sequences, with_deletions)
    return anonymised_sequence


# The dynamic programming algorithm described in Section 4 of [1] - recursion part
def _dp_degree_anonymiser_recursion(degree_sequence, k, C, n, Da, sequences, with_deletions):
    group_degree = None
    if with_deletions:
        group_degree = _median(degree_sequence)
    else:
    ...
    if n < 2 * k:
        ...
        for t in range(np.max([k - 1, n - 2 * k]), n - k):
            ...
            if Da[t] == np.inf:
                cost, sequence = _dp_degree_anonymiser_recursion(
                    degree_sequence[0 : t + 1], k, C, t + 1, Da, sequences, with_deletions
                )
                Da[t] = cost
            ...
            if cost < min_cost:
                ...
                if with_deletions:
                    min_cost_sequence = np.concatenate(
                        (sequence, np.full(np.size(degree_sequence[t + 1 :]), _median(degree_sequence[t + 1 :])))
                    )
                else:
                    min_cost_sequence = np.concatenate(
        ...
        to_return = (
            (min_cost, min_cost_sequence)
            if min_cost < all_in_one_group_cost
            else (all_in_one_group_cost, all_in_one_group_sequence)
        )
        return to_return


# Section 6.2 in [1]
def _priority(degree_sequence, original_G):
    n = len(degree_sequence)
    ...
    while True:
        ...
        # pick a random one
        idx = remaining_vertices[np.random.randint(len(remaining_vertices))]
        v = vd[idx][0]
...
def anonymize_sequence(dv, k, with_deletions):
    degree_sequence, permutation = _sort_dv(dv)
    anonymised_sequence = _dp_degree_anonymiser(degree_sequence, k, with_deletions=with_deletions)

    new_anonymised_sequence = [None] * len(degree_sequence)
    for i in range(len(permutation)):
        new_anonymised_sequence[permutation[i]] = anonymised_sequence[i]

    return new_anonymised_sequence


def k_degree_anonymity(G, k, noise=10, with_deletions=False):
    attempt = 0
